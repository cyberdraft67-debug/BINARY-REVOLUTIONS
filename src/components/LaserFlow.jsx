...
export const LaserFlow = ({
  className, style, wispDensity=1, dpr, mouseSmoothTime=0.0, mouseTiltStrength=0.01,
  horizontalBeamOffset=0.1, verticalBeamOffset=0.0, flowSpeed=0.35, verticalSizing=2.0,
  horizontalSizing=0.5, fogIntensity=0.45, fogScale=0.3, wispSpeed=15.0, wispIntensity=5.0,
  flowStrength=0.25, decay=1.1, falloffStart=1.2, fogFallSpeed=0.6, color='#FF79C6'
}) => {
  ...
  useEffect(() => {
    ...
    const updateMouse=(clientX,clientY)=>{const rect=rectRef.current;if(!rect) return;const x=clientX-rect.left,y=clientY-rect.top,ratio=currentDprRef.current,hb=rect.height*ratio;mouseTarget.set(x*ratio,hb-y*ratio);};
    ...
    canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();pausedRef.current=true;},false);
    canvas.addEventListener('webglcontextrestored',()=>{pausedRef.current=false;scheduleResize();},false);
    let raf=0,lastDprChangeRef=0;
    const dprFloor=0.6,lowerThresh=50,upperThresh=58,dprChangeCooldown=2000;
    const adjustDpr=now=>{
      const elapsed=now-lastFpsCheckRef.current;if(elapsed<750) return;
      const samples=fpsSamplesRef.current;if(samples.length===0){lastFpsCheckRef.current=now;return;}
      const avgFps=samples.reduce((a,b)=>a+b,0)/samples.length;
      let next=currentDprRef.current,base=baseDprRef.current;
      if(avgFps<lowerThresh) next=Math.max(dprFloor,Math.min(currentDprRef.current*0.85,base));
      else if(avgFps>upperThresh&&currentDprRef.current<base) next=Math.max(dprFloor,Math.min(currentDprRef.current*1.1,base));
      if(Math.abs(next-currentDprRef.current)>0.01&&now-lastDprChangeRef>dprChangeCooldown){currentDprRef.current=next;lastDprChangeRef=now;setSizeNow();}
    ...
    const animate=()=>{
      ...
      if(!hasFadedRef.current){fade=Math.min(1,fade+cdt/1.0);uniforms.uFade.value=fade;if(fade>=1) hasFadedRef.current=true;}
      const tau=Math.max(1e-3,mouseSmoothTime),alpha=1-Math.exp(-cdt/tau);
  ...
  useEffect(()=>{
    ...
    u.uWIntensity.value=wispIntensity;u.uFlowStrength.value=flowStrength;u.uDecay.value=decay;
    u.uFalloffStart.value=falloffStart;u.uFogFallSpeed.value=fogFallSpeed;
